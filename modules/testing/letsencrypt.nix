...
# valid zones to be usable for validation and config.opencloud.snakeOilCaCert
...
in {
  options.opencloud.snakeOilCaCert = lib.mkOption {
    type = lib.types.path;
    description = ''
  ...
  config = {
    opencloud.snakeOilCaCert = "${snakeOilCa}/ca.pem";
