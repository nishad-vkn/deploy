...
in {
  ...
  options.opencloud.nixops.snakeoilKeypair = mkOption {
    type = keypairType;
    internal = true;
    description = "The snakeoil VPN keypair for this node.";
  };

  options.opencloud.nixops.resources.sshKeyPairs = mkOption {
  ...
  config = let
    ...
    addrAssignments = map (link: let
      ...
      inherit (nodes.${client}.config.opencloud.nixops) snakeoilKeypair;
  ...
  in {
    ...
    opencloud.nixops.snakeoilKeypair = let
      name = "vpn-${config.networking.hostName}";
    in mkKeyPair name;

    environment.etc."nixops/id_snakeoil" = {
      text = config.opencloud.nixops.snakeoilKeypair.privateKey;
      mode = "0600";
    };

    _module.args.resources = {
      sshKeyPairs = let
        inherit (config.opencloud.nixops.resources) sshKeyPairs;
