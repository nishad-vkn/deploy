...
#      Opencloud-internal options (search for "config.opencloud"), because
...
let
  ...
  mkAddr = opt: fallback:
    if config.opencloud.${opt} != null
    then config.opencloud.${opt}
    else fallback;
  ...
  vhostCfg = let
    inherit (config.opencloud) internalNetConfig mainDevice;
  in internalNetConfig.${mainDevice} or { ip4 = []; ip6 = []; };

in {
  virtualisation.vlans = lib.mkForce [ 1 ];
  opencloud.mainDevice = "eth0";
  ...
  networking.interfaces.${config.opencloud.mainDevice} = {
