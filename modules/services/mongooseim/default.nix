...
let
  ...
  cfg = config.opencloud.services.mongooseim;
  ...
  inherit (config.opencloud.programs.mongooseimctl) ctlTool;
...
in {
  options.opencloud.services.mongooseim = {
  ...
  config = mkMerge [
    { opencloud.services.mongooseim = {
        package = mkDefault pkgs.opencloud.mongooseim;
      };
    }
    (mkIf (cfg.enable && cfg.nodeIp != null) {
      opencloud.erlang-inet.hosts.${nodeHostname} = cfg.nodeIp;
    })
    (mkIf cfg.enable {
      opencloud.services.mongooseim = {
      ...
      opencloud.services.epmd.enable = true;
