...
let
  ...
  zoneWriter = pkgs.opencloud.compileC {
...
in {
  options.opencloud.nsd-zone-writer = {
  ...
  config = lib.mkIf config.opencloud.nsd-zone-writer.enable {
    ...
    users.groups.dynzone = {
      gid = 2023;
      members = config.opencloud.nsd-zone-writer.allowedUsers;
    };
    ...
    opencloud.nsd-zone-writer.command = "/run/nsd-zone-writer/write";
  };
}
