...
let
  ...
  cfg = config.opencloud.services.acme;
  acmetool = pkgs.opencloud.acmetool.bin;
  ...
  validator = pkgs.opencloud.compileHaskell {
    name = "validator";
    source = ./validator.hs;
    ghcflags = [ "-O2" "-Wall" ];
    buildDepends = [ "cereal" "cereal-text" pkgs.opencloud.nexus ];
  };
...
in {
  options.opencloud.services.acme = {
  ...
  config = lib.mkMerge [
    (lib.mkIf cfg.enable {
      ...
      opencloud.conditions.acme.connectable = {
    ...
    (lib.mkIf cfg.dnsHandler.enable {
      opencloud.nsd-zone-writer.enable = true;
      opencloud.nsd-zone-writer.allowedUsers = [ "acme-validator" ];
      ...
      systemd.services.acme-dns-handler = {
        ...
        serviceConfig.ExecStart = let
          inherit (config.opencloud.nsd-zone-writer) command;
