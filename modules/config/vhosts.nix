...
let
  ...
  inherit (config.opencloud) mainDevice;
    ...
    options = {
      ...
      device = mkOption {
        ...
        description = ''
          ...
          device set by <option>opencloud.mainDevice</option>.
        '';
      };
...
in {
  options.opencloud.vhosts = mkOption {
    default = {};
  ...
  options.opencloud.internalNetConfig = mkOption {
    type = types.attrs;
    default = {};
    internal = true;
    description = ''
  ...
  config = mkIf (cfg.vhosts != {}) {
    ...
    opencloud.internalNetConfig = netConfig;
    opencloud.services.acme.domains = acmeConfig;
  };
}
