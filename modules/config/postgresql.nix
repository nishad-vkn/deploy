...
let
  ...
  opt = "opencloud.postgresql.databases";
...
in {
  options.opencloud.postgresql.databases = lib.mkOption {
  ...
  config = lib.mkIf (config.opencloud.postgresql.databases != {}) {
    assertions = let
      ...
      inherit (config.opencloud.postgresql) databases;
    ...
    systemd.services = {
      postgresql-user-roles-init = {
        ...
        '') (getUniqueUsers config.opencloud.postgresql.databases));
      };
    } // lib.mapAttrs' mkDbService config.opencloud.postgresql.databases;
  };
}
