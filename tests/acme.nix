import ./make-test.nix {
  ...
  in {
    dns = { config, pkgs, lib, nodes, ... }: {
      ...
      opencloud.services.acme.dnsHandler = {
        enable = true;
        fqdn = "ns.example.com";
    ...
    webserver = { lib, pkgs, ssl, nodes, ... }: {
      ...
      opencloud.services.lighttpd.enable = true;
      opencloud.services.lighttpd.virtualHosts = lib.singleton {
        type = "static";
        on = "www.example.com";
        docroot = pkgs.runCommand "docroot" {} ''
          mkdir "$out"
      ...
      opencloud.services.acme.enable = true;
      opencloud.services.acme.handlerAddress = "10.0.0.2";
      opencloud.services.acme.handlerDevice = "tun0";
      opencloud.services.acme.domains."example.com" = {
        users = [ "twistd" ];
